...
for file in *.csv; do
    ...
    while IFS=',' read -r rank treatment rest; do
        ...
        # Check if rank is between 0 and 8
        if [[ "$rank" =~ ^[0-8]$ ]]; then
...
for key in "${!counts[@]}"; do
    ...
    if [[ -z "${consolidated[$treatment]}" ]]; then
        consolidated["$treatment"]="rank0,0,rank1,0,rank2,0,rank3,0,rank4,0,rank5,0,rank6,0,rank7,0,rank8,0"
    fi
    ...
    case $rank in
        ...
        8) consolidated["$treatment"]=$(echo "${consolidated[$treatment]}" | sed "s/rank8,0/rank8,$count/") ;;
    esac
done

# Write results to a temporary file for sorting
temp_file=$(mktemp)
echo "Sampling,Regressor,rank0,count0,rank1,count1,rank2,count2,rank3,count3,rank4,count4,rank5,count5,rank6,count6,rank7,count7,rank8,count8" > "$temp_file"

for treatment in "${!consolidated[@]}"; do
    echo "$treatment,${consolidated[$treatment]}" >> "$temp_file"
done

# Sort by `count0` (fourth column, numerically, descending)
